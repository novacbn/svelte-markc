...
const html_stringify = require("rehype-stringify");
...
const {javascript_stringify, markdown_frontmatter, svelte_compile_hast} = require("./plugins");

/**
 * Returns a `Promise<Map<string, unknown>>` of frontmatter key-values parsed from the given contents
 * @param {*} file
 * @param {*} settings
 */
...
async function compile_markdown(file, settings = {}) {
    ...
    processor.use(markdown_parse, settings.parse).use(html_stringify, rehype.stringify);
...
async function compile_markc(file, settings = {}) {
    ...
    reecma = get_reecma_settings(reecma);
    rehype = get_rehype_settings(rehype);
    remark = get_remark_settings(remark);
    ...
    processor.use(markdown_parse, settings.parse).use(javascript_stringify, reecma.stringify);

    use_remark_plugins(processor, remark, false);
    processor.use(mdast_to_hast, remark.mdast2hast);
