...
function get_processor(options) {
    ...
    for (const plugin of remark.plugins) {
        const [transformer, transformer_options] = plugin;

        if (transformer_options) processor.use(transformer, transformer_options);
        else processor.use(transformer);
    }
    ...
    for (const plugin of rehype.plugins) {
        const [transformer, transformer_options] = plugin;
