...
const {replace_arrow_characters, replace_brace_characters} = require("../util");
...
/**
 * Represents a Markdown `unified.Processor` plugin for converting special characters in codeblocks to HTML entity codes
 */
function markdown_code_escape() {
    return (ast) => {
        visit(ast, "code", (node) => {
            let {value} = node;

            value = replace_arrow_characters(value);
            value = replace_brace_characters(value);
